...
class BranchSummaryGenerator:
    def __init__(self, branch_name: str, days_back: int = 7, branch_only: bool = False):
        ...
        self.since_date = (datetime.now() - timedelta(days=days_back)).strftime(
            "%Y-%m-%d"
        )
    ...
    def get_branch_commits(self) -> List[dict]:
        ...
        if exit_code != 0:
            print(
                f"Error getting commits for branch '{self.branch_name}': {output}",
                file=sys.stderr,
            )
            return []
    ...
    def generate_summary(self) -> str:
        ...
        if pr_info:
            status_emoji = (
                ...
                else "🔄" if pr_info["state"] == "OPEN" else "❌"
            )
        ...
        for category, category_commits in categories.items():
            ...
            for commit in category_commits:
                ...
                if ":" in clean_message and any(
                    prefix in clean_message.lower()
                    for prefix in [
                        "feat:",
                        "fix:",
                        "refactor:",
        ...
        if len(commits) <= 10:
            ...
            for commit in commits:
                category_emoji = self.categorize_commit_message(
                    commit["message"]
                ).split()[0]
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate branch summary for Clockify task"
    )
    parser.add_argument(
        "branch", nargs="?", help="Branch name (current branch if not specified)"
    )
    parser.add_argument(
        "--days",
        "-d",
    ...
    if not args.branch:
        ...
        if result.returncode == 0:
            ...
            print(
                "Error: Could not determine current branch. Please specify branch name.",
                file=sys.stderr,
            )
            sys.exit(1)
